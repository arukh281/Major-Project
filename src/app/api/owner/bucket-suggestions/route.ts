...
- **In-person education & studios** (music lessons, instruments, tutoring, dance, arts classes, etc.): Customers review **the space** (practice rooms, noise, lighting, comfort) and **money** (lesson prices, packages, value)—unless the description clearly says **online-only**, **non-commercial**, or **free/community**. For typical paid, **on-site** instruction, include **all four**: \`staff_service\`, \`food_drinks\`, \`cleanliness_ambience\`, and \`pricing_trust\`. Still **exclude** \`delivery_ops\` and \`stock_items_orders\` unless shipping, couriers, retail inventory, or order/rental fulfillment is described.
- **staff_service** vs **pricing_trust**: Include **pricing_trust** for any normal paid business (tuition, lesson fees, memberships, refunds). Omit only when the text clearly implies no payment (free workshops, volunteer-only, etc.).
- Aim for about **2–5** buckets for a focused business; **four** is common for on‑site professional services. Use **all six** only when the description clearly mixes retail or delivery with on‑site service (e.g. shop + shipping + in‑store experience).
